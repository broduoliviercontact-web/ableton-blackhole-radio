...
const shaderVisual = resolveVisual({ visualization: 'shader-radio', shaderPreset: 'signal-aurora', shaderQuality: 'high' })
assert(shaderVisual.visualization === 'shader-radio' && shaderVisual.shaderPreset === 'signal-aurora' && shaderVisual.shaderQuality === 'high', 'shader radio resolve preset/quality')
...
assert(BUILT_IN_SCENES.length === 17, 'scenes: 17 scenes systeme')
...
assert(new Set(BUILT_IN_SCENES.map((scene) => scene.visual.visualization)).size === 17, 'scenes: une scene par visualisation')
